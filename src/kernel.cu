...
__global__ void clique_kernel(int *A, int N, char *device_graph, float *device_pheromone, curandState *states, unsigned int seed) {
...
extern "C" int anthill(char **graph, int N, int M){
	...
	for(int i = 0; i < M; i++){
		...
		clique_kernel<<<block_size,1>>>(results, N, device_graph, device_pheromone, states, (unsigned int)time(NULL));
		cudaMemcpy(host_results, results, block_size*sizeof(int), cudaMemcpyDeviceToHost);
